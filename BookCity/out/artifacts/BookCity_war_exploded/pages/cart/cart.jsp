	...
	<div id="main">
		<table>
			...
			<c:if test="${empty sessionScope.cart.itemMap}" >
				<tr>
					<td colspan="5"><a href="index.jsp">亲，当前购物车为空！，快跟伙伴们去浏览商品吧！！！</a></td>
				</tr>
			</c:if>
			<c:if test="${not empty sessionScope.cart.itemMap}" >
				<c:forEach items="${sessionScope.cart.itemMap}" var="entry">
					<tr>
						...
						<td>
                            <input bookId="${entry.value.id}" class="updateCount" type="number" value="${entry.value.count}" style="width: 80px">
                        </td>
						<td>${entry.value.name}</td>
		...
		<c:if test="${not empty sessionScope.cart.itemMap}" >
			<div class="cart_info">
				...
				<span class="cart_span"><a id="cleanCar" href="cartServlet?action=clear">清空购物车</a></span>
